...
let peoples : string[] = ['jim' , 'kim'];
let peoples2 : {member1 : string , member2 : string} = {member1 : 'kim' , member2 : 'park'};
...
let members : (number | string)[] = [1,2,3 , 'sda'];
let membersObj :{a : (string|number) , b : number}  = {a :"123" , b : 12};


//? any Type
//? 변수에 타입을 할당안하면 기본적으로 any타입이다.
//? 타입 쉴드를 없애버린다. 그러므로 위험하다.
let name2  : any //? 모든 자료형을 허용해준다. 타입실드 해제 문법이다. 일반자바스크립트 변수이기때문에 타입스크립트를 사용하는 의미가 없기때문에 잘 사용하지 않는다.

//? unknownType - anyType보다 안전하다.

let name3 : unknown //? unknown타입 모든 자료형을 허용해줌 . any와 유사한 타입 
...
function funVoid(x? : number) : void{
    //! return 1+1 err
}
...
function fun2(x : string | number) : number{
    if(typeof x === 'number'){
        x = x.toString();
        return x.length;
    }
...
function merryme(salary : number , house : boolean , appeal : string) : string | void{
    let score : number = (house ? 500 : 0);
    score = score+(appeal === '상' ? 100 : 0);
    if(score+salary >= 600) return "결혼가능";
}
...
function 내함수(x : number | string){
    let array : number[] = []; //? array는 숫자만 들어가는 배열
    ...
    //? Assertion
    array[0] = x as number  //? assertion문법 (타입 덮어쓰기) number로 인식하게됨.
...
//? 문제 2
function makefun(obj : {subject : string | string[]}) : string{
...
//* 타입이 너무 길고 복잡할때 타입변수 만들기

//? type키워드로 타입을 만드는걸 type alias라고 한다.  만들때는 일반변수와 다르게 대문자로 시작하자 또는 뒤에 Type을 붙혀도되고
//? 타입변수는 재정의 불가능

type Animal = string | number | undefined;
let animals : Animal = 'cat' //? union Type  
 
//? 객체 타입 
type Animal2 = {name : string , age : number};
let animals2 : Animal2 = {name : 'dog' , age : 15};
...
//? object타입 합치기 & 연산자를 이용하여
type PositionX = { x : number};
type PositionY = { y : number};
// {x : number , y : number};  이렇게 합치고 싶은거다.
type Position = PositionX & PositionY  //? {x : number , y : number}; extend된다. 
...
const girl : { readonly name : string , age : number} = { name : 'ember' , age :25 };
...
//? 문제 4
type Type4 = {name : string , tel : number , email : string , child : boolean};
